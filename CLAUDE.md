...
- **Part B — Content:** `src/data/grammar/b2.ts` (new — 120 exercises, 10 topics), real B2 tips in `tips.ts` (20), `topicTipMap.ts` B2 section (10 entries), 10 B2 reading passages in `passages.ts`, 50 B2 sentences in `sentenceBuilder.ts`.
...
- [2026-04-07] Phase 38 Part A complete — B2 vocabulary done. `src/data/vocabulary/b2.ts` replaced with 2,123 Aspekte Neu B2 words (IDs b2_0001 → b2_2123), sourced from b2_aspekte.txt. Wired into `src/data/vocabulary/index.ts` (`B2: B2_WORDS`). B2 flashcards now show real vocabulary. Part B (grammar, tips, topicTipMap, passages, sentences) pending.
